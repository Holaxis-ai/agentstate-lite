...
timestamp: '2026-08-05T20:58:17.651Z'
---
# Purpose
...
A live active record or unexpired cooldown causes immediate render-only fallback. An unavailable/failed worker atomically replaces only its matching unexpired active record with cooldown; a successful worker writes cache before removing only its matching active record. A recognized stale active record is replaced in one continuously occupied atomic step by its original 24-hour cooldown—never via quarantine/absence—so even interruption preserves the one-start-per-attempt-window bound. An expired cooldown is removed in a cleanup-only visit; a later eligible visit acquires. Malformed, foreign, symlinked, non-regular, wrong-owner, wrong-mode, or oversized coordination state is never removed.

No code performs read/compare/unlink on the fixed path. Token-scoped removal after successful cache publication, an unused post-claim cache hit, spawn failure, or expired cooldown atomically renames the fixed entry to a unique quarantine path, validates what was captured, and restores a mismatched capture when possible. Live/stale active-to-cooldown transitions use continuous atomic replacement instead, so quarantine never opens an attempt-window gap. Any worker whose token is no longer the matching fixed unexpired active record aborts before network and again before cache commit.
...
3. Builder adds a bounded barrier/IPC multi-process fixture proving one atomic claim winner, continuous stale-active-to-cooldown replacement, and the paused-parent interleaving: parent B reads stale cache, worker A publishes cache/removes active, B claims, B's post-claim cache revalidation releases without spawning. No sleeps/shared append logs. It also adds deterministic in-process worker tests and built-entry probes proving valid/invalid hidden routing, zero work in JSON/suppressed paths, and no public registration without live npm.
