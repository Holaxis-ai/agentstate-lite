...
timestamp: '2026-08-05T20:58:17.455Z'
---
# Purpose
...
- Active/cooldown transitions fail closed and never block rendering. A live active record or live cooldown means render-only. An unavailable worker atomically replaces only its matching unexpired active record with cooldown; a recognized stale active record is likewise replaced in one continuously occupied atomic step by its original attempt-window cooldown, never via an absent fixed-path gap. A successful worker writes the cache before removing only its matching active record. An expired cooldown is removed by a cleanup-only visit, and only a later eligible visit may acquire. Foreign/malformed/symlinked/non-regular/oversized/unsafe-owner-or-mode state is never removed automatically. Fixed-path removal uses a token-validated quarantine transition rather than read-then-unlink, and a worker revalidates a matching unexpired active lease before network work and immediately before cache commit.
- Refresh: after winning an active claim, the parent freshly re-reads/revalidates the cache before spawning; if another worker published a fresh cache meanwhile, it releases its unused claim and starts no worker. Otherwise it starts one detached invocation of the exact current executable's private `__update-refresh-v1` worker, stdio ignored, no retries, using the same 2,000 ms/1 MiB network primitive. Parent returns without waiting and removes only its matching active record if spawn itself fails. The worker derives current identity and latest track itself; missing/invalid private-worker arguments are silent zero-work exits.
