...
// The per-host tests below are GENERATED by iterating SKILL_HOST_ROOTS — which means silently
// deleting a host from that array silently deletes its test too, and the suite stays green. This
// is the one assertion that catches THAT regression: the array's contents are typed out here as
// literals, independent of the constant under test, so removing (or adding) a host requires a
