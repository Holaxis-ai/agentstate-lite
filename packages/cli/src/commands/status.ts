...
import { parseRegistration, VIEW_ENTRY_PREFIX } from "@agentstate-lite/core/page";
...
instances missing a declared inbound expectation ('missing_expected_links'), plus a dangling-entry
lint over registered View docs, legacy Page included ('dangling_view_entries'). Duplicate-id
detection is not offered: an id IS its storage path, so ids are structurally unique.
...
Category semantics (one line each):
  ...
  dangling_view_entries  A registered View doc — legacy Page included, via the same recognition
                      ...
                      Present (even at 0) whenever the bundle carries any recognized
                      registration; absent otherwise.
...
export async function status(argv: string[], deps: Partial<StatusCliDeps> = {}): Promise<void> {
  ...
  // dangling_view_entries (tasks/view-entry-dangling-lint): recognized View registrations
  // (legacy Page included) — core's `parseRegistration`, the SAME predicate the ui
  // launcher/loopback server consume, so
  // recognition is convention-independent by construction — whose declared `entry` names a blob
  // that does not exist. A valid registration pointing at a never-promoted, typo'd, or
  // since-deleted key mints a view that serves nothing; nothing on the write path checks this
  // (deliberately — a lint also catches hand-written docs, external bundles, and post-hoc blob
  // deletions). Existence derives from the two prefix-scoped listings batched above, never a
  // per-registration existence probe.
  ...
  // Present — even at 0 — whenever the bundle carries at least one recognized registration (the
  // conformance_debt gating idiom): "all entries resolvable" is a report worth
  // seeing, while a registration-free bundle's output stays byte-identical to before this lint
  // existed (the byte-identity pins in status.test.ts).
  if (registrationCount > 0) out.dangling_view_entries = danglingViewEntries.total;
