...
export async function status(argv: string[], deps: Partial<StatusCliDeps> = {}): Promise<void> {
  ...
  // Unresolved links + the inbound edge set (reused below for orphans — never a per-doc
  // backlinks() call, which would be N whole-bundle traversals over data already in hand).
  ...
  for (const doc of docs) {
    for (const l of parseLinksFromDoc(doc)) {
      ...
      // "From OTHER concept docs" — a self-link does not rescue a doc from orphan status.
      if (l.to !== doc.id) inbound.add(l.to);
    }
  }
  ...
  // convention) so a clean bundle's report stays a short summary, not eight empty categories.
