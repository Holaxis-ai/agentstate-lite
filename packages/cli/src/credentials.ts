...
export async function saveCredentials(
  ...
  // Enforce dir perms even if it pre-existed with looser bits (mkdir mode is masked by umask).
  await chmod(dir, DIR_MODE);

  const path = credentialsPath(home);
  const tmpPath = join(dir, `.${CRED_FILE_NAME}.${randomBytes(8).toString("hex")}.tmp`);
  ...
  try {
    // Atomic on the same filesystem: okf-config.json is replaced in one step, so a reader
    // sees either the old complete file or the new complete file — never a partial write.
