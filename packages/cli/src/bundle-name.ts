...
//   (a) EXPLICIT — the well-known doc {@link BUNDLE_NAME_DOC_ID} (`docs/bundle`): its
//       frontmatter `name` (set by editing the markdown), else its `title` (settable via plain
//       `doc write docs/bundle --type Doc --title <name>` / `doc update docs/bundle --title
//       <name>` — no new verb or flag). A committed doc, so the name syncs to teammates.
...
// backend (filesystem for every current consumer); a missing/malformed doc is the common case and
// simply falls through. Consumers: the `ui` command's `/__ui/config` (shell header + bridge
// `hello.bundle.name`) and home/session-start's bundle block — and the workspace catalog is
// expected to consume THIS function too (the pairing note on
...
/** The frontmatter field that wins over `title` on the well-known doc (see the module header). */
 ...
 * throws — an absent, unreadable, or malformed well-known doc (and a backend `read` that throws
 * for any reason) degrades silently down the chain to a purely path-derived name.
 */
export async function deriveBundleDisplayName(bundle: Bundle): Promise<string> {
  try {
    const doc = await readDoc(bundle, BUNDLE_NAME_DOC_ID);
    const explicit =
      nonEmptyString(doc.frontmatter[BUNDLE_NAME_FIELD]) ?? nonEmptyString(doc.frontmatter.title);
